...
mod lexer;
mod strtab;
use self::{diagnostics::Diagnostics, lexer::TokenKind};
...
///
/// NOTE: this kind of error represents an exception inside the compiler. It does NOT represent lexical or semantic issues of the MiniJava source code
...
fn run_compiler(cmd: &CliCommand) -> Result<(), Error> {
    match cmd {
        ...
        CliCommand::LexerTest { path } => {
            ...
            let stderr = termcolor::StandardStream::stdout(termcolor::ColorChoice::Always);
            let mut stdout = io::stdout();
            let strtab = strtab::StringTable::new();
            let diagnostics = Diagnostics::new(Box::new(stderr));
            let lexer = lexer::Lexer::new(ascii_file.iter(), &strtab, &diagnostics);
