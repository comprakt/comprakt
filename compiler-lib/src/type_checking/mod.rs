//! This module implements type checking on the AST.
//! The type checking produces an instance of `type_system::TypeSystem`, which
//! is later used by the `firm` module for IR graph construction.
//!
//!
//! ## `$` Types (`builtin_types`)
//!
//! MiniJava suffers from the fact that its specification was stripped down
//! from the Java language report.
//! A good example is the handling of the main method's arguments:
//! `String[] args` must not be used, and according to the spec, `String[]`
//! within the main method's parameter list is not a type.
...
//! result of the discussion we had in the
//! lecture.
//!
